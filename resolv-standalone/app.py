"""RESOLV - Image Resolution Lab (standalone).

A small, self-contained Flask app to change image resolution locally:
...
Upscaling calls the external Real-ESRGAN ncnn-vulkan binary plus its
``.param``/``.bin`` model files. Place the binary next to this file and the
models inside ``models/`` (see models/PUT_MODELS_HERE.txt). Downscale and noop
work without any binary or model.
...
# Real-ESRGAN ncnn-vulkan binary, searched next to this file.
...
def run_upscale(input_path: str, output_path: str, model: str, target_scale: float) -> None:
    ...
    if exe is None:
        raise FileNotFoundError(
            "Binary realesrgan tidak ditemukan. Taruh 'realesrgan-ncnn-vulkan.exe' "
            "(Windows) atau 'realesrgan-ncnn-vulkan' (Linux/macOS) sejajar dengan app.py."
        )
    if not model_files_present(model):
        raise FileNotFoundError(
            f"File model '{model}.param' / '{model}.bin' tidak ada di folder models/. "
...
if __name__ == "__main__":
    if find_exe() is None:
        print("=" * 64)
        print(" INFO: binary realesrgan belum ada di folder ini.")
        print(" Downscale (<1x) & noop (1x) tetap jalan; upscale (>1x) butuh")
        print(" binary + model. Lihat models/PUT_MODELS_HERE.txt.")
        print("=" * 64)
